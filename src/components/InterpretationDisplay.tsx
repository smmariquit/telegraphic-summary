...
function Stage({
  ...
  return (
    <section ref={ref} className="stage rise scroll-mt-[var(--space-md)]" aria-labelledby={id} aria-current={focus ? "step" : undefined}>
      <div className="grid gap-[var(--space-sm)] sm:grid-cols-[minmax(0,5rem)_minmax(0,1fr)]">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-[var(--space-2xs)] max-w-[var(--measure)] text-muted [text-wrap:pretty]">{lede}</p>
          {children}
        </div>
      </div>
    </section>
    ...
    <div className="space-y-[var(--space-2xl)]">
      ...
      {step >= 2 && (
        <Stage
          ...
          focus={step === 2}
        >
          <ol className="mt-[var(--space-md)] divide-y divide-rule border-y border-rule">
            {groups.map((g, i) => (
              <li
                key={g.pattern}
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-[var(--space-2xs)] max-w-[var(--measure)] text-[length:var(--text-sm)] text-muted [text-wrap:pretty]">
                    {facts[i]?.replace(/^[^:]*:\s*/, "")}
                  </p>
                </div>
              </li>
          ...
          {step === 2 && (
            <div className="mt-[var(--space-lg)]">
              ...
              <p className="mt-[var(--space-2xs)] text-[length:var(--text-sm)] text-muted">
      ...
      {step >= 3 && (
        <section className="stage rise scroll-mt-[var(--space-md)]" aria-labelledby="stage-3-0" aria-busy={proseLoading} aria-current="step">
          <div className="grid gap-[var(--space-sm)] sm:grid-cols-[minmax(0,5rem)_minmax(0,1fr)]">
            <p className="stage__num" aria-hidden="true">
              3.0
            </p>
            <div className="min-w-0 max-w-[var(--measure)]">
              <h2 id="stage-3-0" className="text-[length:var(--text-xl)]">
                Translate into sentences
              </h2>
              <p className="mt-[var(--space-2xs)] text-muted [text-wrap:pretty]">
                One sentence per line of the summary, the one that answers the objective first. Then the sentences
                expanded into a paragraph.
              </p>

              {proseLoading && (
                <div role="status" aria-label="Writing sentences" className="mt-[var(--space-lg)] space-y-[var(--space-sm)]">
                  {groups.map((g) => (
                  ...
                  <p className="text-[length:var(--text-sm)] text-muted">Writing…</p>
                </div>
              )}
              ...
              {prose && !proseLoading && (
                <div className="rise">
                  ...
                  <ol className="mt-[var(--space-xs)] divide-y divide-rule border-y border-rule">
                  ...
                  <p className="mt-[var(--space-sm)] text-[length:var(--text-sm)] text-muted [text-wrap:pretty]">
                    <span className="font-mono">[cite]</span> marks where the book expects a source from the literature
                    (p82, p84). The model has none and did not invent one. Fill those in yourself.
                  </p>
                </div>
              )}
            </div>
